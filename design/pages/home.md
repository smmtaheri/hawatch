...
Home باید در چند ثانیه به کاربر بگوید هواچ برای دیدن هوای مقصد و برنامهٔ مسیر است. تصمیم اصلی کاربر انتخاب مقصد برای بررسی است؛ مقصد می‌تواند از طریق جست‌وجو یا مقصدهای محبوب انتخاب شود.

## ۲. مسیر ورود و خروج

- ورود: آدرس ریشهٔ محصول `/` یا لینک برند از هر صفحه.
- خروج اصلی: انتخاب مقصد و رفتن به `/destination/{slug}`.
...
۳. جست‌وجوی مقصد شامل input و دکمهٔ «جست‌وجو».
۴. heading «مقصدهای محبوب».
...
- input جست‌وجو: مقصد را با keyboard و متن فارسی می‌پذیرد؛ submit با Enter نیز فعال است.
- دکمهٔ «جست‌وجو»: query را normalize می‌کند و نتیجه یا empty state را نشان می‌دهد.
