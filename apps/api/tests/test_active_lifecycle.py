...
def test_inactive_route_hidden_from_lists_siblings_and_direct_endpoint(api_client, tochal):
    route = Route.objects.get(slug="touchal-kalkchal")
    route.is_active = False
    route.save(update_fields=["is_active"])

    dest = api_client.get("/api/v1/destinations/touchal/forecast/").json()
    assert "touchal-kalkchal" not in {item["slug"] for item in dest["related_routes"]}
    assert "touchal-kalkchal" not in {item["slug"] for item in dest["destination"]["routes"]}

    siblings = api_client.get("/api/v1/routes/touchal-darband/forecast/").json()["route"]["siblings"]
    assert "touchal-kalkchal" not in {item["slug"] for item in siblings}

    assert api_client.get("/api/v1/routes/touchal-kalkchal/forecast/").status_code == 404
...
def test_inactive_destination_hides_routes_and_returns_404(api_client, tochal):
    ...
    assert api_client.get("/api/v1/destinations/touchal/forecast/").status_code == 404
    assert api_client.get("/api/v1/routes/touchal-darband/forecast/").status_code == 404
    assert api_client.get("/api/v1/points/sarband/forecast/").status_code == 404
...
def test_related_routes_require_active_route_and_destination(api_client, tochal):
    ...
    Route.objects.filter(slug="touchal-darband").update(is_active=False)
    body = api_client.get("/api/v1/points/goleband/forecast/").json()
    assert body["related_routes"]
    assert "touchal-darband" not in {item["slug"] for item in body["related_routes"]}
    assert "touchal-kalkchal" in {item["slug"] for item in body["related_routes"]}
...
def test_ingest_excludes_inactive_route_and_destination(tochal):
    ...
    assert "sarband" not in list(ingestible_weather_points().values_list("slug", flat=True))

    Route.objects.filter(points__weather_point=point).update(is_active=True)
    Destination.objects.filter(slug="touchal").update(is_active=False)
    assert "sarband" not in list(ingestible_weather_points().values_list("slug", flat=True))
...
def test_search_hides_inactive_route_destination_and_point(tochal):
    ...
    assert any(item["slug"] == "sarband" for item in search_suggestions(query="سربند"))
    ...
    Destination.objects.filter(slug="touchal").update(is_active=True)
    Route.objects.filter(slug="touchal-darband").update(is_active=True)
    WeatherPoint.objects.filter(slug="sarband").update(is_active=False)
    rebuild_search_index()
    assert not any(item["slug"] == "sarband" for item in search_suggestions(query="سربند"))
