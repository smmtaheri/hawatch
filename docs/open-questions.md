...
| OQ-005 | PASS | loading در Home، Point، Route و Point چه UI و timeout/fallback داشته باشد؟ | تصمیم قطعی: skeleton با حفظ layout. | در state contract گزارش‌ها ثبت شد؛ جزئیات timeout آینده است. |
...
| OQ-009 | PASS | مدل forecast روزها، period و unitها چیست؟ | پنجرهٔ ۷روزه، timezone رسمی `Asia/Tehran` و چهار پنجرهٔ ثابت با سه برش دوساعته در API/UI محلی پیاده شده است. | نیمه‌شب ۰۰–۰۶، صبح ۰۶–۱۲، ظهر ۱۲–۱۸ و شب ۱۸–۲۴؛ انتخاب پیش‌فرض بر اساس ساعت تهران است. |
