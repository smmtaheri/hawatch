...
آبشار، دشت، جنگل یا روستای شناخته‌شده)، آن را در همان catalog با
...
هر مقصد جدید URL یا لیست دستی در frontend نسازید. Route پیادهٔ کامل و مستند برای
مقصدهای پیمایشی ترجیح دارد و در صورت وجود باید اضافه شود، اما نبود ترک معتبر
مانع انتشار خود نقطه نیست: مقصد را با `routes: {}` به‌صورت point-only منتشر
کنید و بعداً با یافتن ترک مناسب همان catalog را تکمیل کنید. وضعیت pending فقط
برای Route ناقص یا فاقد شواهد است، نه برای صفحهٔ مقصد. waypointهای فنی و نقاط
صرفاً بین‌راهی با `seo_indexable: false` همچنان در route/API/search می‌مانند اما
وارد Hub و sitemap نمی‌شوند. قبل از import duplicate مختصات/نام را در کل
catalog و دیتابیس بررسی کنید و نوع واقعی عارضه را ثبت کنید.
...
- اگر مسیر پیادهٔ معتبر نداریم، `routes: {}` کافی است؛ این برای مقصدهای شاخص
  آینده نیز مجاز است و نبود Route نباید مانع ساخت WeatherPoint یا صفحهٔ مقصد
  شود. مسیر آفرودی، خودرو، دوچرخه یا صخره‌نوردی را hiking route ثبت نکنید؛ در
  صورت یافتن ترک پیادهٔ معتبر، Route را بعداً به همان مقصد اضافه کنید.
...
- بلوک اختیاری `seo` فقط وقتی اضافه شود که copy آن کوتاه، منبع‌دار و واقعاً
  مفید است. کلیدهای مجاز `title`، `description` و `content` هستند؛ نبود بلوک
  قالب عمومی پویا را فعال می‌کند. متن حدسی، keyword-stuffed یا تکراری را وارد
  catalog نکنید.
